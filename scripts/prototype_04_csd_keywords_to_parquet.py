"""
prototype_04_csd_keywords_to_parquet.py
=======================================
IQVIA CSD Keywords xlsx -> Parquet report-month partitions.

Reference policy mirrored from jw-market:
- SQL DDL source: sql/schema_iqvia.sql, stg_iqvia_csd_keywords_raw
- CSD row id: SHA256(keywords|source_file|source_sheet|source_row_id)[:32]
- Header detection and raw_row_json construction follow
  jw-market/etl/iqvia_csd_loader.py and iqvia_transform.py.
- Keywords keep full source rows as JSON; there is no observations_json column.
- Sparse policy: empty raw rows are skipped.

Prototype storage:
- 1 row = 1 raw source row, matching stg_iqvia_csd_keywords_raw.
- Output partition key is report_month (YYYY-MM), written to
  parquet/csd_keywords/YYYY-MM.parquet.
- Core DDL columns are preserved; prototype helper columns are period and
  source_files.

Usage:
    python3 scripts/prototype_04_csd_keywords_to_parquet.py \\
        --input-dir "data/IQVIA/CSD/Keyword (고객=의사에게 전달한 메세지)" \\
        --output-dir parquet/csd_keywords
"""
...
def iter_csd_keyword_rows(xlsx_path: Path, ingested_at: str | None = None) -> Iterable[dict[str, Any]]:
    ...
    for sheet_name, rows in iter_workbook_sheets(xlsx_path):
        ...
        for source_row_id, values in enumerate(rows[header_row:], start=header_row + 1):
            ...
            yield {
                "csd_row_id": make_csd_row_id("keywords", xlsx_path.name, sheet_name, source_row_id),
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-dir",
        default="data/IQVIA/CSD/Keyword (고객=의사에게 전달한 메세지)",
        help="CSD Keywords xlsx 폴더",
    )
    parser.add_argument("--output-dir", default="parquet/csd_keywords")
    parser.add_argument("--db-path", default="staging_csd_keywords.duckdb")
    ...
    print("=" * 72)
    print("IQVIA CSD Keywords -> Parquet (report-month partitions)")
    print("=" * 72)
    ...
    for index, xlsx_path in enumerate(files, start=1):
        ...
        inserted_for_file = 0
        for row in iter_csd_keyword_rows(xlsx_path, ingested_at=ingested_at):
            chunk.append(row)
            inserted_for_file += 1
