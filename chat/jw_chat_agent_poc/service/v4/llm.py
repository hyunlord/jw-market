...
_REASONING_EFFORTS = frozenset({"low"})
_USE_CLIENT_REASONING_EFFORT = object()


def normalize_reasoning_effort(value: str | None) -> str | None:
    normalized = value.strip().lower() if value else None
    if normalized is not None and normalized not in _REASONING_EFFORTS:
...
class GenOSV4Client:
    ...
    def complete(
        ...
        *,
        ...
        reasoning_effort: str | None | object = _USE_CLIENT_REASONING_EFFORT,
    ) -> str:
        client = self._client
        ...
        return _chat_completion_with_token_cap(
            ...
            reasoning_effort=(
                ...
                if reasoning_effort is _USE_CLIENT_REASONING_EFFORT
...
def synthesizer_client() -> GenOSV4Client:
    ...
    default_budget = math.ceil(
        max(policy.max_synthesis_budget_s, policy.insight_lane_timeout_s)
    )
    ...
    return GenOSV4Client(
        ...
        thinking_level=os.environ.get("V4_SYNTHESIZER_THINKING_LEVEL", "LOW"),
        reasoning_effort=os.environ.get("V4_SYNTHESIZER_REASONING_EFFORT"),
    )
