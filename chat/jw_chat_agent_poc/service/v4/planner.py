...
from jw_chat_agent_poc.common.periods import relative_span, requested_month_range
...
from jw_chat_agent_poc.service.v4.axis_shaping import question_axes
...
def _planner_messages(
    ...
    *,
    ...
    history = project_recent_turns(turns, limit=10)
    ...
    return [
        {
            ...
            "content": (
                ...
                "For a disease question, use the disease condition as the clinical search area and leave "
                "clinical_query_specs.ingredients empty unless the user explicitly names a drug or ingredient. "
                "Disease-to-treatment expansion is controlled by the deterministic entity registry; do not "
                "invent or enumerate treatment ingredients. "
                ...
                "Populate answer_contract.question_core with one sentence stating the user's core question. "
                "Translate every explicit or expanded answer obligation into answer_contract.required_items; "
                "each item needs a stable short id, a Korean ask, and kind data or reading. Put the entities "
                "resolved by this interpretation in answer_contract.resolved_entities. Do not silently omit "
                "an obligation merely because a source may fail. "
...
def _anchor_relative_years(
    ...
    if span is not None and span[1] != "년":
        return plan
...
def _limit_first_wave_queries(plan: PlannerOutput) -> PlannerOutput:
    ...
    for source, queries in plan.tool_queries.items():
        ...
        selected = unique if source in {"hira", "clinicaltrials"} else unique[:limit]
...
def _attach_lossless_contracts(question: str, plan: PlannerOutput) -> PlannerOutput:
    ...
    merged_shape = supplied_shape.model_copy(
        update={
            ...
            "period_from": deterministic_shape.period_from or supplied_shape.period_from,
            "period_to": deterministic_shape.period_to or supplied_shape.period_to,
            "period_count": deterministic_shape.period_count or supplied_shape.period_count,
        }
    )
    return plan.model_copy(
        update={
            ...
            "answer_contract": merge_interpretation_contract(
                derive_answer_contract(
                    question,
                    merged_shape,
...
def _clinical_search_area(question: str, query: str) -> str:
    ...
    if any(token in lowered for token in ("질환", "상병", "disease", "condition")):
        return "condition"
    if disease_query_from_question(question) and any(
        token in question for token in ("환자", "유병", "질환", "상병")
    ):
        return "condition"
    return "intervention"
...
def _requested_answer_shape(question: str) -> RequestedAnswerShape:
    ...
    horizon_match = re.search(
        r"최근\s*\d{1,2}\s*(?:년\s*(?:\d{1,2}\s*(?:개월|달)|반)?|개월|달|분기)",
        question,
    )
    ...
    period_from, period_to, period_count = _requested_period_bounds(question)
...
def _requested_period_bounds(
    question: str,
) -> tuple[str | None, str | None, int | None]:
    today = _current_kst_date()
    anchor = f"{today.year:04d}-{today.month:02d}"
    requested = requested_month_range(question, anchor)
    ...
    if explicit_years:
        ...
        count = (end_year - start_year) * 12 + end_month
        return f"{start_year:04d}-01", f"{end_year:04d}-{end_month:02d}", count
...
def _patent_query(question: str) -> str:
    terms = list(dict.fromkeys(question.split()))
    if "특허" not in question:
        terms.append("특허")
    if "만료" not in question:
        terms.append("만료")
    return " ".join(terms)
...
def _direct_answer_sources(question: str) -> tuple[SourceName, ...]:
    ...
    if has_explicit_file_source_comparison(question):
        return ("document", *explicit_file_comparison_sources(question))
    if has_file_axis_reference(question):
        return ("document",)
    axes = question_axes(question)
    if "patient_statistics" in axes and "market_total" in axes:
        return ("hira", "mart")
    if "patient_statistics" in axes:
        return ("hira",)
    if any(token in lowered for token in ("환자", "상병", "급여")):
        return ("hira",)
    if any(
