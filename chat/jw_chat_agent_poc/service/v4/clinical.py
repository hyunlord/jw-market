...
def clinical_relevance_status(decision: ClinicalRelevanceDecision) -> str:
    if decision.keep:
        return "직접 관련 확인"
    if (
        decision.reason_code == "missing_required_ingredient_token"
        and decision.matched_tokens
    ):
        return "성분 관련"
    return "직접 관련 여부 미확인"
...
def compile_clinical_query(concept: ClinicalTrialConcept) -> CompiledClinicalQuery:
    if concept.search_area == "condition":
        # Planner ingredient suggestions describe possible treatments, not the
        # disease entity. Condition searches must stay bound to the condition
        # wording so a patient-count question cannot fan out into drug OR terms.
        terms = _clean_terms(concept.source_queries)
    else:
        terms = _clean_terms(concept.ingredients)
        if not terms:
            raise ValueError(
                "clinical intervention query requires a resolved ingredient"
            )
    if not terms:
    ...
    # Country and status are answer conditions, not retrieval conditions.
    # Keeping them on the concept lets post-retrieval aggregation report
    # "all N / matching M" without shrinking the source population.
