...
_MISSING_CELL_LABELS = frozenset({"원천 미제공", "미제공", "not provided", "n/a"})
...
def cell(value: object) -> str:
    ...
    if not rendered or rendered.casefold() in _MISSING_CELL_LABELS:
        return "-"
    for label in sorted(_MISSING_CELL_LABELS, key=len, reverse=True):
        rendered = re.sub(re.escape(label), "-", rendered, flags=re.IGNORECASE)
    return (
        rendered.replace("\\", "\\\\")
        .replace("|", "\\|")
        .replace("_", "\\_")
