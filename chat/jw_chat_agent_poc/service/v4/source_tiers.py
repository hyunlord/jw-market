...
_EMBEDDED_PERIOD_MODIFIER_RE = re.compile(
    r"(?:최근\s*)?\d{1,3}\s*년(?:\s*\d{1,2}\s*개월)?|"
    r"(?:최근\s*)?\d{1,3}\s*(?:개월|분기|개년)"
)
_EMBEDDED_PERIOD_SUFFIX_RE = re.compile(
    r"^(?P<base>.+?)\s+(?P<period>(?:최근\s*)?\d{1,3}\s*년"
    r"(?:\s*\d{1,2}\s*개월)?|(?:최근\s*)?\d{1,3}\s*(?:개월|분기|개년))$"
)
...
_PERIOD_MODIFIER_RE = re.compile(
    r"^(?:최근|올해|금년|작년|전년|당해|현재|요즘|"
    r"\d{1,3}\s*(?:개년|년|개월|분기|주|일)|"
    r"최근\s*\d{1,3}\s*(?:개년|년|개월|분기|주|일))$"
)
...
def sanitize_planner_entities(
    ...
    *,
    ...
    for raw_entity in plan.requested_answer_shape.entities:
        ...
        parenthetical_candidate: str | None = None
        ...
        if parenthetical is not None:
            ...
            if base and candidate:
                parenthetical_candidate = candidate
        ...
        if entity in _GENERIC_ENTITY_NOUNS:
            reason = "generic_noun"
        elif _PERIOD_MODIFIER_RE.fullmatch(entity):
            reason = "period_modifier"
        ...
        if canonical and canonical != entity:
            ...
            entity = canonical
        if (
            parenthetical_candidate
            and parenthetical_candidate not in parenthetical_candidates
        ):
            parenthetical_candidates.append(parenthetical_candidate)
...
def _completion_entities(plan: PlannerOutput) -> tuple[str, ...]:
    ...
    return tuple(
        dict.fromkeys(
            _strip_embedded_period_suffix(entity)
            for entity in candidate_entities
            if entity
            and ":" not in entity
            and not _is_render_axis_token(entity)
            and not _is_question_fragment(entity)
            and not _is_period_modified_duplicate(entity, candidate_entities)
            and entity not in _GENERIC_ENTITY_NOUNS
        )
    )


def _is_period_modified_duplicate(
    entity: str,
    candidates: tuple[str, ...],
) -> bool:
    return any(
        entity != base
        and entity.startswith(base)
        and _EMBEDDED_PERIOD_MODIFIER_RE.fullmatch(entity[len(base) :].strip())
        is not None
        for base in candidates
    )


def _strip_embedded_period_suffix(entity: str) -> str:
    match = _EMBEDDED_PERIOD_SUFFIX_RE.fullmatch(" ".join(entity.split()))
    return match.group("base").strip() if match is not None else entity
...
def entity_completion_rows(
    ...
    required_sources = set(
        plan.answer_contract.required_sources or plan.answer_sources
    )
    for entity in entities:
