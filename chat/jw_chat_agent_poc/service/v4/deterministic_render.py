...
from jw_chat_agent_poc.service.v4.render_file_source import (
    render_file_source_comparison,
)
...
def render_deterministic_facts(
    ...
    *,
    ...
    comparison = render_file_source_comparison(
        plan,
        evidence_sets,
    ...
    source_notices, source_notice_bindings = (
        _source_failure_notices(plan, evidence_sets, lane_execution=lane_execution)
        if _source_notices_enabled(plan, profile, evidence_sets)
        or (
            lane_execution is not None
            and any(source in lane_execution for source in DOCUMENT_LEDGER_SOURCES)
        )
        else ((), ())
    )
    source_tiers = {
    ...
    for evidence_set in rendered_sets:
        period_count = (
            plan.answer_contract.required_period_count
            if plan.answer_contract is not None
            and plan.answer_contract.answer_shape is AnswerShape.TIME_SERIES
            else None
        )
...
def _render_set(
    ...
    *,
    ...
    if source == "hira":
        ...
        return render_hira_statistics(evidence_set, question=question)
    if source == "mart":
        return render_market(evidence_set, period_count=period_count)
    if source == "nedrug":
        return render_nedrug(evidence_set, observed_on=observed_on)
    if source == "web":
...
def _source_failure_notices(
    ...
    *,
    ...
    if lane_execution is not None:
        bindings = source_notice_bindings_from_lane_execution(lane_execution)
        return tuple(str(binding["notice"]) for binding in bindings), bindings
    notices: list[str] = []
