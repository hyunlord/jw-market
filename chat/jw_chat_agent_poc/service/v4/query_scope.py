...
WEB_SOURCE_CALL_LIMIT = 2
# Compatibility constant for older trace readers. HIRA no longer applies this
# fixed cap because every derived disease-code query must be accounted for.
HIRA_SOURCE_CALL_LIMIT = 24
...
_HIRA_CODE_RE = re.compile(r"(?<![A-Z0-9])[A-Z]\d{2}(?:\.?\d{1,2})?(?![A-Z0-9])")
...
def _balanced_hira_queries(queries: tuple[str, ...]) -> tuple[str, ...]:
    ...
    valid = tuple((match, query) for match, query in parsed if match is not None)
    unparsed = tuple(query for match, query in parsed if match is None)
    if not valid:
        return queries
    codes = tuple(dict.fromkeys(match.group("code").upper() for match, _query in valid))
    years = tuple(dict.fromkeys(match.group("year") for match, _query in valid))
    if len(codes) < 2 or len(years) < 2:
        return tuple(query for _match, query in valid) + unparsed
    by_axis = {
        (match.group("code").upper(), match.group("year")): query
        for match, query in valid
    }
    balanced = tuple(
        by_axis[(code, year)]
        for year in years
    ...
    valid_queries = tuple(query for _match, query in valid)
    prioritized = balanced if len(balanced) == len(valid_queries) else valid_queries
    return (*prioritized, *unparsed)


def apply_source_call_cap(plan: Any) -> Any:
    """Apply source caps while preserving lossless HIRA and CT query sets."""
    ...
    for source, queries in plan.tool_queries.items():
        ...
        if source == "web":
            source_limit = WEB_SOURCE_CALL_LIMIT
        else:
            source_limit = SOURCE_CALL_LIMIT
        valid_hira_queries = tuple(
            query for query in selection_order if _HIRA_CODE_RE.search(query)
        )
        selected = (
            valid_hira_queries or selection_order
            if source == "hira"
            else unique
            if source == "clinicaltrials"
            else selection_order[:source_limit]
        )
        updates[source] = selected
        previous_omitted = (
            tuple(previous.omitted_queries.get(source, ())) if previous is not None else ()
        )
        preserve_first_wave_scope = (
            source not in {"hira", "clinicaltrials"}
            and bool(previous_omitted)
            and len(unique) <= source_limit
        )
        requested[source] = (
        ...
        if preserve_first_wave_scope:
            ...
            omitted[source] = tuple(
                query for query in selection_order if query not in selected
            )
            if source == "hira":
                unexecuted_reasons[source] = "invalid_hira_query_format"
