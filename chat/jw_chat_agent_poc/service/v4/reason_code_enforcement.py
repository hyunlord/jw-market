...
def _repair_patent_status(
    ...
    for result in results:
        ...
        if isinstance(raw_records, list):
            ...
            brand_scope_applied = kr_lane.get("brand_scope_applied") is True or any(
                record.get("brand_scope_match") in {True, False}
                for record in scoped_records
            )
            records.extend(
                record
                for record in scoped_records
                if not brand_scope_applied or record.get("brand_scope_match") is True
            )
    if not records:
        return None
    registered = sum(
