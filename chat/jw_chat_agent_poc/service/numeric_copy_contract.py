...
def _trusted_internal_payload(result: Mapping[str, Any]) -> str:
    ...
    if isinstance(calls, list):
        for call in calls:
            ...
            canonical_source = str(call.get("source") or "").strip().casefold()
            if canonical_source == "mart" or any(
                source in source_text for source in _TRUSTED_INTERNAL_SOURCES
            ):
...
def _numeric_call_payload(call: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(payload, Mapping):
        nested_calls = payload.get("calls")
        if isinstance(nested_calls, list):
            selected["payload"] = {
                **payload,
                "calls": [
                    _numeric_call_payload(nested)
                    for nested in nested_calls
                    if isinstance(nested, Mapping) and not _call_failed(nested)
                ],
            }
    return selected
