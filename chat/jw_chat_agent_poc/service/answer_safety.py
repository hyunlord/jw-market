...
def ensure_file_absence_statement(question: str, answer: str, file_context: str) -> str:
    ...
    if not missing:
        return answer
    received_count = len(re.findall(r"(?m)^\[\d+\]", context))
    lines = "\n".join(
        f"수신된 상위 {received_count}개 청크에서는 {target}을(를) 확인하지 못했습니다."
...
_DOCUMENT_ABSENCE_RE = re.compile(
    r"(?:확인되지\s*않|확인하지\s*못|찾지\s*못|자료\s*없|정보.{0,20}없)",
    re.IGNORECASE,
)
_DOCUMENT_SCOPE_REPLACEMENTS = (
    re.compile(
        r"제공된\s+[^.\n]{1,180}?\s+문서\s+데이터를[^,.\n]{0,80}(?:결과)?\s*,?\s*",
        re.IGNORECASE,
    ),
    re.compile(r"(?:제공된\s+)?문서(?:\([^)]*\))?\s*(?:내)?에서", re.IGNORECASE),
    re.compile(r"현재\s+보유\s+원천에서", re.IGNORECASE),
)


def scope_document_absence_claims(text: str, *, received_count: int) -> str:
    """Scope document-absence prose to the chunks that retrieval actually returned."""

    if not text:
        return text
    scope = f"수신된 상위 {max(received_count, 0)}개 청크에서는"
    parts = re.split(r"(?<=[.!?])(\s+)", text)
    for index in range(0, len(parts), 2):
        sentence = parts[index]
        if (
            scope in sentence
            or not _DOCUMENT_ABSENCE_RE.search(sentence)
            or not ("문서" in sentence or "보유 원천" in sentence)
        ):
            continue
        scoped = sentence
        for pattern in _DOCUMENT_SCOPE_REPLACEMENTS:
            scoped, count = pattern.subn(scope, scoped, count=1)
            if count:
                break
        if scoped == sentence:
            scoped = f"{scope} {sentence.lstrip()}"
        parts[index] = scoped
    return "".join(parts)
