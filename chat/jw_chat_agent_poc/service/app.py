...
from jw_chat_agent_poc.service.v4.inspection import build_document_inspection_detail
...
from jw_chat_agent_poc.service.sse_presenter import (
    SSE_PRESENTER_ENV,
    normalize_section_label_fragments,
    normalize_section_labels,
    selected_sse_presenter,
)
from jw_chat_agent_poc.service.sse_protocol import strip_structured_markdown_tables
...
def _uploaded_result_from_delegated_payload(
    payload: tuple[Any, ...],
) -> UploadedFileSearchResult:
    if len(payload) == 4:
        delegated_context, source_items, has_active_file, deterministic_answer = payload
        sql_trace: tuple[dict[str, str], ...] = ()
        tool_detail: Mapping[str, Any] = {}
    elif len(payload) == 5:
        (
            delegated_context,
            source_items,
        ...
        tool_detail = raw_tool_detail if isinstance(raw_tool_detail, Mapping) else {}
    return UploadedFileSearchResult(
        ...
        errors=tuple(
            dict.fromkeys(
                str(error)
                for error in tool_detail.get("errors", ())
                if str(error).strip()
            )
        ),
        file_source_items=tuple(source_items),
        has_active_file=bool(has_active_file),
        deterministic_answer=str(deterministic_answer or ""),
        sql_trace=tuple(sql_trace),
        rag_query=str(tool_detail.get("rag_query") or ""),
        rag_queries=tuple(
            str(query)
            for query in tool_detail.get("rag_queries", ())
            if str(query).strip()
        ),
        rag_top_k=(
            tool_detail.get("rag_top_k")
            if isinstance(tool_detail.get("rag_top_k"), int)
            else None
        ),
        rag_top_k_source=str(tool_detail.get("rag_top_k_source") or ""),
        record_context=str(tool_detail.get("record_context") or ""),
        prompt_context_limit=(
            tool_detail.get("prompt_context_limit")
            if isinstance(tool_detail.get("prompt_context_limit"), int)
            else None
        ),
        rag_trace=(
            dict(tool_detail.get("rag_trace"))
            if isinstance(tool_detail.get("rag_trace"), dict)
            else {}
        ),
        sql_detail=(
            dict(tool_detail.get("sql_detail") or {})
            if isinstance(tool_detail.get("sql_detail"), Mapping)
            else {}
        ),
        detail_source_items=tuple(
            dict(item)
            for item in tool_detail.get("detail_source_items", ())
...
def _run_v4_final_answer(
    ...
    answer_ready_callback: Callable[[str, str, tuple[str, ...]], None] | None = None,
    section_ready_callback: Callable[[dict[str, Any]], None] | None = None,
    file_context: str | None = None,
    load_session_files: bool = True,
) -> FinalAnswer:
    uploaded = (
        _uploaded_result_from_delegated_payload(
            _delegated_file_context(
                file_comparison_leg_query(question),
                conversation_id,
                file_context,
                include_all_files=True,
                include_tool_detail=True,
            )
        )
        if load_session_files
        else UploadedFileSearchResult(
            file_context="",
            file_sources=(),
            errors=(),
    ...
    document_result = (
        ...
        if has_active_file or delegated_context or source_items
        else None
    )
    ...
    runtime = _get_v4_runtime()
    if answer_ready_callback is not None and _accepts_answer_ready_callback(runtime.answer):
        runtime_kwargs["answer_ready_callback"] = answer_ready_callback
    if section_ready_callback is not None and _accepts_callback(
        runtime.answer, "section_ready_callback"
    ):
        runtime_kwargs["section_ready_callback"] = section_ready_callback
    if document_result is not None:
        runtime_kwargs["supplemental_results"] = (document_result,)
    answer = runtime.answer(question, **runtime_kwargs)
    answer_text = _restore_file_display_labels(answer.text, source_items)
...
def _accepts_answer_ready_callback(answer: Callable[..., Any]) -> bool:
    return _accepts_callback(answer, "answer_ready_callback")


def _accepts_callback(answer: Callable[..., Any], name: str) -> bool:
    try:
...
def _restore_file_display_labels(
    text: str,
    source_items: Iterable[Mapping[str, Any]],
) -> str:
    """Restore source-owned sheet labels after generated prose normalization."""

    restored = text
    for item in source_items:
        sheet_name = str(item.get("sheet_name") or "")
        normalized = " ".join(sheet_name.split())
        if sheet_name and normalized and sheet_name != normalized:
            restored = restored.replace(normalized, sheet_name)
    return restored
...
def _v4_file_scope_answer(
    ...
    uploaded = _uploaded_result_from_delegated_payload(
        _delegated_file_context(
            question,
            conversation_id,
            file_context,
...
def _v4_file_scope_answer_from_retrieval(
    ...
    *,
    ...
    if has_active_file or delegated_context or source_items:
        ...
        no_document = document_result is not None and document_result.status == "no_document"
        if no_document:
            file_answer = FinalAnswer(
                text="세션에 연결된 문서를 찾지 못했습니다.",
                charts=[],
        ...
        elif document_result is not None and is_document_overview_question(question):
            file_answer = FinalAnswer(
        ...
        all_public_sources = _project_public_file_sources(source_items)
        narrative_file_names = {
            str(source.get("file_name") or "") for source in public_sources
        }
        records = tuple(
            {
                "record_id": "FILE-"
                ...
                **source,
                "narrative_used": str(source.get("file_name") or "")
                in narrative_file_names,
            }
        ...
        if document_result is not None:
            document_records = document_result.payload.get("records", ())
            rendered_document_ids = tuple(
                str(record.get("record_id") or "")
                for record in document_records
                if isinstance(record, Mapping) and record.get("record_id")
            )
            trace["inspection_detail"] = build_document_inspection_detail(
                question,
                document_result,
                rendered_record_ids=rendered_document_ids,
            )
...
def create_app(
    *,
    ...
    def start_history_projection_worker() -> None:
        ...
        projection.start()
        start_history = getattr(history, "start", None)
    ...
    def stop_history_projection_worker() -> None:
        stop_history = getattr(history, "stop", None)
        if callable(stop_history):
            flushed = stop_history()
            if flushed is False:
                LOGGER.error("conversation trace shutdown flush did not complete")
        projection.stop()
    ...
    @app.get(
        "/chat/detail/{conversation_id}/{response_id}/{item_key:path}",
        summary="Return one user-owned archived detail item",
    )
    def chat_detail(
        conversation_id: str,
        response_id: str,
        ...
        portal_user_id = projection_context.portal_user_id
        if portal_user_id is None:
            raise HTTPException(status_code=401, detail="portal user identity is required")
        load_detail_trace = getattr(history, "load_detail_trace", None)
        if not callable(load_detail_trace):
            raise HTTPException(status_code=503, detail="detail archive is unavailable")
        trace = load_detail_trace(conversation_id, response_id, portal_user_id)
        if not isinstance(trace, Mapping):
            raise HTTPException(status_code=404, detail="detail item not found")
        detail = resolve_detail_item(trace, item_key)
        if detail is None:
            raise HTTPException(status_code=404, detail="detail item not found")
        detail = hydrate_web_detail(detail)
    ...
    def chat_answer(
        ...
        transport_answer = project_answer_for_transport(final_answer.text, final_answer.trace)
        return ChatAnswer(
            text=transport_answer.text,
            charts=final_answer.charts,
            trace=transport_answer.trace,
    ...
    *,
    ...
    with sink_context:
        ...
        with trace_span("conversation_state_load", "in-memory conversation state lookup"):
            state = store.conversations.get_or_create(conversation_id)
        file_conversation_id = _file_inventory_conversation_id(
        ...
        explicit_file_reference = has_file_reference(effective_question)
        explicit_upload_reference = bool(
            _EXPLICIT_UPLOAD_FILE_REFERENCE_RE.search(effective_question)
        )
        file_probe_started = time.perf_counter()
        active_file_for_scope = provided_file or bool(
            conversation_id
            and not explicit_upload_reference
            and has_active_uploaded_file(file_conversation_id)
        )
        has_file = active_file_for_scope or explicit_file_reference
        file_probe_elapsed_ms = (time.perf_counter() - file_probe_started) * 1000
        schema_probe_started = time.perf_counter()
        file_schema_columns = (
            fetch_uploaded_file_schema_columns(file_conversation_id)
            if has_file and not provided_file and not explicit_file_reference
            else ()
        )
        schema_probe_elapsed_ms = (time.perf_counter() - schema_probe_started) * 1000
        file_overviews = (
            fetch_uploaded_file_overviews(file_conversation_id)
        ...
        app_scope_kwargs = {
            ...
            "has_file": active_file_for_scope,
        ...
        if routing_resolution.unresolved_reference and context_scope is not ContextScope.MIXED:
            ...
            result = _answer_mixed_parallel(
                ...
                file_context,
                file_question=file_question,
                file_conversation_id=file_conversation_id,
                use_direct_agent_loop=use_direct_agent_loop,
            )
            ...
            if _has_active_upload and effective_question.strip():
                result = _answer_mixed_parallel(
                    store,
                    market_scope_resolver,
                    agent_factory,
                ...
                context_scope = ContextScope.MIXED
            elif not effective_question.strip() and _has_file_signal(documents, delegated_file_context):
        ...
        result = {
            **result,
            ...
            "_sec12_input_policy_decision": input_policy_decision,
            "_upload_restore": {
                "projected_conversation_id": state.conversation_id,
                "file_conversation_id": file_conversation_id,
                "source_id_resolved": file_conversation_id != state.conversation_id,
            },
        }
...
def _file_inventory_conversation_id(
    history: ConversationHistoryStore | None,
    conversation_id: str,
) -> str:
    resolver = getattr(history, "resolve_source_conversation_id", None)
    if not callable(resolver):
        return conversation_id
    source_id = resolver(conversation_id)
    return source_id or conversation_id
...
def _answer_mixed_parallel(
    ...
    *,
    ...
    file_conversation_id: str | None = None,
    prefetched_file_payload: tuple[
        str | None,
        tuple[dict[str, Any], ...],
        bool,
        str,
    ...
    dependent_questions = _dependent_file_market_questions(question)
    dependent_mode = dependent_questions is not None
    if dependent_questions is None:
        market_question = _mixed_market_question(question)
        resolved_file_question = file_question or question
    else:
        dependent_file_question, market_question = dependent_questions
        resolved_file_question = file_question or dependent_file_question
    executor = ThreadPoolExecutor(
        max_workers=(
            1
            if prefetched_file_payload is not None or dependent_mode
            else 2
        ),
        thread_name_prefix="mixed-m1",
    )
    file_future = (
        None
        if prefetched_file_payload is not None
        else executor.submit(
            _delegated_file_context,
            resolved_file_question,
            file_conversation_id or conversation_id,
            file_context,
            include_tool_detail=True,
        )
    )
    ...
    dependent_stage2_started = False
    try:
        with stage(None, "mixed_file_leg", "uploaded file retrieval"):
            if prefetched_file_payload is not None:
                file_payload = prefetched_file_payload
            else:
                assert file_future is not None
                try:
                    file_payload = file_future.result(
                        timeout=min(timeout_s, _remaining_seconds(deadline))
                    )
                    ...
                    file_payload = (None, (), True, "", ())
        if dependent_mode and _file_payload_has_grounded_value(
            file_payload,
            question=resolved_file_question,
        ):
            ...
            dependent_stage2_started = True
        with stage(None, "mixed_market_leg", "market fact retrieval"):
            if market_future is None:
                market_result = _dependent_market_blocked_result(question)
            else:
                try:
                ...
                except Exception as exc:  # noqa: BLE001 - preserve partial mixed response
                    LOGGER.warning("mixed market leg failed type=%s", type(exc).__name__)
                    market_result = _mixed_market_failure_result(question)
    finally:
        executor.shutdown(wait=False, cancel_futures=True)

    uploaded_file = _uploaded_result_from_delegated_payload(file_payload)
    context = uploaded_file.file_context or None
    source_items = uploaded_file.file_source_items
    deterministic_answer = uploaded_file.deterministic_answer
    file_sql_trace = uploaded_file.sql_trace
    ...
    if not context:
        no_document = any(bool(item.get("no_document")) for item in source_items)
        file_result["mixed_leg_state"] = "no_document" if no_document else "failed"
        file_result["mixed_leg_error"] = (
            "세션에 연결된 문서를 찾지 못했습니다."
            if no_document
            else "첨부 문서 근거를 가져오지 못했습니다."
        )
    ...
    if source_items:
        combined["file_source_items"] = [dict(item) for item in source_items]
    ...
    diagnostics.update(
        {
            "mode": "mixed_m1_dependent" if dependent_mode else "mixed_m1_parallel",
            "dependent_stage2_started": dependent_stage2_started,
...
_DEPENDENT_FILE_MARKET_RE = re.compile(
    r"^(?P<file>.+?)\s*(?P<action>"
    + "|".join(re.escape(action) for action in _DEPENDENT_FILE_ACTIONS)
    + r")\s*(?P<market>.+)$",
    re.IGNORECASE,
)


def _dependent_file_market_questions(question: str) -> tuple[str, str] | None:
    normalized = re.sub(r"\s+", " ", question).strip()
    if not _EXPLICIT_UPLOAD_FILE_REFERENCE_RE.search(normalized):
        return None
    match = _DEPENDENT_FILE_MARKET_RE.match(normalized)
    if match is None:
        return None
    file_part = match.group("file").strip(" ,")
    market_part = re.sub(
        r"^그\s*결과(?:값)?(?:을|로|과|와)?\s*",
        "",
        match.group("market").strip(" ,"),
    )
    if not file_part or not market_part:
        return None
    file_question = f"{file_part} {_DEPENDENT_FILE_ACTIONS[match.group('action')]}"
    entity_part = re.sub(
        r"^(?:(?:업로드|첨부)(?:한)?\s*)?(?:(?:내|이)\s*)?"
        r"(?:파일|엑셀|xlsx|xlsm|csv|문서)(?:에서|의)?\s*",
        "",
        file_part,
        flags=re.IGNORECASE,
    )
    entity_match = re.match(r"([0-9A-Za-z가-힣][0-9A-Za-z가-힣._-]*)", entity_part)
    entity = entity_match.group(1) if entity_match is not None else ""
    if entity and entity.casefold() not in market_part.casefold():
        market_part = f"{entity} {market_part}"
    return file_question, market_part


_FILE_VALUE_FAILURE_MARKERS = (
    "확인할 수 없습니다",
    "확인하지 못했습니다",
    "변환하지 못했습니다",
    "조건 일치 0건",
...
def _file_payload_has_grounded_value(
    payload: tuple[Any, ...],
    *,
    question: str = "",
) -> bool:
    uploaded = _uploaded_result_from_delegated_payload(payload)
    if any(bool(item.get("no_document")) for item in uploaded.file_source_items):
        return False
    trace_states = {
        str(item.get("status") or "").strip().casefold()
        for item in uploaded.sql_trace
        if isinstance(item, Mapping)
    }
    sql_detail = uploaded.sql_detail
    executed_sql = str(sql_detail.get("executed_sql") or "").strip()
    detail_rows = sql_detail.get("rows")
    result_facts = sql_detail.get("result_facts")
    facts = result_facts if isinstance(result_facts, Mapping) else {}
    facts_rows = facts.get("rows")
    facts_values = facts.get("values")
    has_sql_value = bool(
        executed_sql
        and (
            (isinstance(detail_rows, (list, tuple)) and bool(detail_rows))
            or (isinstance(facts_rows, (list, tuple)) and bool(facts_rows))
            or (isinstance(facts_values, Mapping) and bool(facts_values))
            or facts.get("value") is not None
            or bool(facts.get("periods"))
        )
    )
    if has_sql_value:
        return True

    answer_text = uploaded.deterministic_answer.strip()
    context_text = uploaded.file_context.strip()
    spreadsheet_target = bool(_SPREADSHEET_TARGET_RE.search(question))
    has_document_record = any(
        not bool(item.get("metadata_only"))
        and any(
        ...
        for item in (*uploaded.detail_source_items, *uploaded.file_source_items)
    )
    if (
        not spreadsheet_target
        and (has_document_record or uploaded.record_context.strip())
        and context_text
    ):
        return True

    combined_text = f"{answer_text}\n{context_text}"
    if trace_states & _FILE_VALUE_FAILURE_STATES or any(
        marker in combined_text for marker in _FILE_VALUE_FAILURE_MARKERS
    ):
        return False
    ...
    if spreadsheet_target:
        return bool(
            "ok" in trace_states
            and any(
                str(item.get("stage") or "").strip().casefold() == "execution"
                and str(item.get("status") or "").strip().casefold() == "ok"
                for item in uploaded.sql_trace
                if isinstance(item, Mapping)
            )
            and answer_text
    ...
    return bool((has_document_record or uploaded.record_context.strip()) and context_text)


def _dependent_market_blocked_result(question: str) -> dict:
    answer = "파일에서 1단계 값을 확인하지 못해 후속 시장 조회를 실행하지 않았습니다."
    return {
        "question": question,
        "answer": answer,
        "sources": [],
...
def _delegated_file_context(
    ...
    *,
    ...
    file_source_items = uploaded.file_source_items if uploaded is not None else ()
    ...
    tool_detail = (
        {
            "rag_query": uploaded.rag_query,
            "rag_queries": uploaded.rag_queries,
            "rag_top_k": uploaded.rag_top_k,
            "rag_top_k_source": uploaded.rag_top_k_source,
            "record_context": uploaded.record_context,
            ...
            "detail_source_items": tuple(
                dict(item) for item in uploaded.detail_source_items
            ),
        }
    ...
    if uploaded is not None:
        uploaded_record_context = (
            uploaded.record_context or uploaded.file_context
        ).strip()
        if uploaded_record_context:
            record_contexts.append(uploaded_record_context)
    provided = (file_context or "").strip()
    if provided:
        contexts.append(provided)
        record_contexts.append(provided)
    if include_tool_detail and record_contexts:
        tool_detail["record_context"] = "\n\n".join(
            dict.fromkeys(record_contexts)
        )
    if not contexts:
        result = (
            None,
        ...
        return (*result, tool_detail) if include_tool_detail else result
...
_EXPLICIT_UPLOAD_FILE_REFERENCE_RE = re.compile(
    r"(?:업로드|첨부|올린|파일|pdf|ppt|파워포인트|word|워드|엑셀|xlsx|xlsm|csv)",
    re.IGNORECASE,
)
...
def _attach_file_context(
    ...
    if not context and not file_source_items:
        return result
    copied = dict(result)
    if context:
    ...
    if context and "document" not in sources:
...
def _v4_sse_events(
    ...
    streamed_prefix = ""
    streamed_section_ids: set[str] = set()
    source_conversation_id = conversation_id or uuid4().hex
    pending_turn = _begin_conversation_history(
        history_store,
        session_id=None,
    ...
    def emit_answer_ready(
        ...
        emitted.wait(timeout=2.0)

    def emit_section_ready(event: dict[str, Any]) -> None:
        emitted = threading.Event()
        events.put({"_event_type": "section_ready", **event, "emitted": emitted})
        emitted.wait(timeout=2.0)
        ...
        try:
            with actor_user_scope(projection_context.portal_user_id):
                final_answer = _run_v4_final_answer(
                    ...
                    answer_ready_callback=emit_answer_ready,
                    section_ready_callback=emit_section_ready,
                    load_session_files=conversation_id is not None,
                )
                result_box["answer"] = _answer_with_history_notice(
                    final_answer,
                    _record_conversation_history(
                        history_store,
                        session_id=None,
            ...
            _mark_incomplete_conversation_history(
                history_store,
                pending_turn=pending_turn,
                question=question,
                projection_context=projection_context,
    ...
    try:
        ...
        while not completed.is_set() or not events.empty():
            try:
                ...
                continue
            if event.get("_event_type") == "answer_ready":
                streamed_prefix = str(event.get("text") or "")
                answer_conversation_id = str(event.get("conversation_id") or "")
                first_answer_frame = True
                try:
                    for frame in _sse_initial_text_events(
                        conversation_id=answer_conversation_id,
                        sources=tuple(event.get("sources") or ()),
                        text=streamed_prefix,
                    ):
                        if first_answer_frame and frame.startswith(
                            ...
                            record_first_answer_delta(
                                ...
                                output_bytes=len(frame.encode("utf-8")),
                            )
                            first_answer_frame = False
                        yield frame
                finally:
                    emitted = event.get("emitted")
                    if isinstance(emitted, threading.Event):
                        emitted.set()
                continue
            if event.get("_event_type") == "section_ready":
                section_id = str(event.get("section_id") or "")
                status = str(event.get("status") or "streaming")
                section_text = str(event.get("text") or "")
                if status == "complete":
                    section_text = strip_structured_markdown_tables(section_text)
                    section_text = normalize_section_labels(section_text)
                payloads: list[dict[str, Any]] = []
                checks = event.get("checks")
                raw_paragraphs = event.get("paragraphs")
                raw_section_paragraphs = (
                    [item for item in raw_paragraphs if isinstance(item, Mapping)]
                    if status == "complete"
                    and isinstance(raw_paragraphs, Sequence)
                    and not isinstance(raw_paragraphs, (str, bytes))
                    else []
                )
                raw_paragraph_texts = [
                    strip_structured_markdown_tables(str(paragraph.get("text") or ""))
                    for paragraph in raw_section_paragraphs
                ]
                normalized_paragraph_texts = normalize_section_label_fragments(
                    raw_paragraph_texts
                )
                paragraphs: list[tuple[Mapping[str, Any], str]] = []
                for paragraph, paragraph_text in zip(
                    raw_section_paragraphs, normalized_paragraph_texts, strict=True
                ):
                    if paragraph_text:
                        paragraphs.append((paragraph, paragraph_text))
                for index, (paragraph, paragraph_text) in enumerate(paragraphs):
                    payload: dict[str, Any] = {
                        ...
                        "delta": (
                            ...
                            if paragraph.get("paragraph_start")
                            else " "
                        )
                        + paragraph_text,
                        "status": (
                            "complete" if index == len(paragraphs) - 1 else "streaming"
                        ),
                    }
                    ...
                    if isinstance(evidence_group, Mapping):
                        emitted_group = dict(evidence_group)
                        members = emitted_group.get("members")
                        evidence_ids = (
                            [
                                str(member.get("evidence_id") or "")
                                for member in members
                                if isinstance(member, Mapping)
                                and member.get("evidence_id")
                            ]
                ...
                if not payloads:
                    ...
                    evidence_group = event.get("evidence_group")
                    if isinstance(evidence_group, Mapping):
                        payload["evidence_group"] = dict(evidence_group)
                    payloads.append(payload)
                if isinstance(checks, Mapping):
                    payloads[-1]["checks"] = dict(checks)
                try:
                    for payload in payloads:
                        yield _sse_json_event("answer_section_delta", payload)
                    if status == "complete" and section_id:
                        streamed_section_ids.add(section_id)
                finally:
                    emitted = event.get("emitted")
                    if isinstance(emitted, threading.Event):
                        emitted.set()
        ...
        latency_probe = get_latency_probe(final_answer.conversation_id)
        if latency_probe is not None:
            latency_probe.checkpoint("app.runtime_return")
        if latency_probe is not None:
            latency_probe.checkpoint("app.history_notice")
    except Exception as exc:  # noqa: BLE001 - preserve the existing SSE error contract
        LOGGER.exception("V4 answer generation failed")
        yield _sse_json_event(
            "error",
            _public_runtime_error_payload(exc, partial_answer=bool(streamed_prefix)),
        )
        yield "event: done\ndata: error\n\n"
        return
    yield from _sse_events_from_final_answer(
        final_answer,
        streamed_prefix=streamed_prefix,
        streamed_section_ids=tuple(sorted(streamed_section_ids)),
    )


def _public_runtime_error_payload(
    _exc: Exception,
    *,
    ...
    if partial_answer:
        return {
            ...
            "message": (
                "확인된 조회 결과는 먼저 표시했지만, 후속 해설과 최종 정리를 "
                "완료하지 못했습니다."
            ),
        }
    return {
        "type": "AnswerUnavailable",
        "status": "runtime_error",
        "message": "답변 생성 중 오류가 발생해 요청 결과를 확인하지 못했습니다.",
    }
...
def _sse_events_from_final_answer(
    ...
    *,
    streamed_prefix: str = "",
    streamed_section_ids: tuple[str, ...] = (),
):
    ...
    if latency_probe is not None:
        latency_probe.checkpoint(
            ...
            output_bytes=transport_answer.transport_bytes,
            object_count=len(transport_answer.trace),
        )
    try:
        yield from selected_sse_presenter().final_answer_events(
            ...
            trace=transport_answer.trace,
            streamed_prefix=streamed_prefix,
            streamed_section_ids=streamed_section_ids,
        )
        if latency_probe is not None:
            latency_probe.checkpoint(
                "transport.sse_emit",
                input_bytes=transport_answer.transport_bytes,
                output_bytes=transport_answer.transport_bytes,
            )
    finally:
        finish_latency_probe(final_answer.conversation_id)
...
def _begin_conversation_history(
    history_store: ConversationHistoryStore | None,
    *,
    session_id: str | None,
    conversation_id: str,
    ...
    if history_store is None:
        return None
    begin_turn = getattr(history_store, "begin_turn", None)
    ...
    try:
        ...
        LOGGER.exception("failed to create incomplete chat conversation history")
        return None
    return pending if isinstance(pending, PendingTurn) else None
...
def _record_conversation_history(
    ...
    *,
    ...
    # The persistence and SSE projections must share one response identifier.
    # Live answers historically had no trace_id, so assigning it here keeps the
    # same stable key on the mutable trace both projections already share.
    if isinstance(final_answer.trace, MutableMapping):
        if pending_turn is not None:
            final_answer.trace["trace_id"] = pending_turn.trace_id
        elif not final_answer.trace.get("trace_id"):
            final_answer.trace["trace_id"] = uuid4().hex
    if history_store is None:
        return TurnPersistOutcome(status=PERSIST_STATUS_SKIPPED, reason="no_store")
    try:
        replay_answer = project_answer_for_transport(final_answer.text, final_answer.trace)
        archival_trace = archive_trace_for_detail(
            replay_answer.trace,
            final_answer.trace,
            portal_user_id=(
                projection_context.portal_user_id if projection_context is not None else None
        ...
        if pending_turn is not None:
            outcome = history_store.complete_turn(
                pending_turn,
                question_text=question,
                answer_text=replay_answer.text,
                trace=archival_trace,
                timing=final_answer.timing,
                sources=final_answer.sources,
                charts=final_answer.charts,
                projection_context=projection_context,
...
def _mark_incomplete_conversation_history(
    history_store: ConversationHistoryStore | None,
    *,
    ...
    if history_store is None or pending_turn is None:
        return TurnPersistOutcome(status=PERSIST_STATUS_SKIPPED, reason="no_pending_turn")
    fail_turn = getattr(history_store, "fail_turn", None)
    if not callable(fail_turn):
        return TurnPersistOutcome(status=PERSIST_STATUS_SKIPPED, reason="legacy_store")
    try:
        outcome = fail_turn(
            pending_turn,
            question_text=question,
        ...
        LOGGER.exception("failed to mark chat conversation history incomplete")
        return TurnPersistOutcome(
...
_DOCUMENT_FILE_SUFFIXES = (".pdf", ".doc", ".docx", ".ppt", ".pptx", ".txt")
_SPREADSHEET_FILE_SUFFIXES = (".xlsx", ".xlsm", ".xls", ".csv")
...
def _deterministic_mixed_file_answer(
    ...
    context = str(result.get("file_context") or "").strip()
    deterministic = str(result.get("deterministic_file_answer") or "").strip()
    public_context = _public_mixed_file_evidence(context)
    preference = _file_surface_preference(str(result.get("question") or ""), result)
    if preference == "document":
        has_spreadsheet = any(
            _file_name_has_suffix(item, _SPREADSHEET_FILE_SUFFIXES)
            for item in _file_source_items(result)
        )
        if has_spreadsheet:
            evidence = public_context or deterministic
        else:
            evidence = deterministic or public_context
    elif preference == "spreadsheet":
        evidence = deterministic or public_context
    elif deterministic and public_context:
        evidence = (
            f"## 업로드 문서 검색 결과\n{public_context}\n\n"
            f"## 업로드 스프레드시트 조회 결과\n{deterministic}"
        )
    ...
    file_name = _mixed_file_name(result, preference)
    page = _mixed_file_page(result, context, preference)
    source = f"출처: 업로드 문서 · {file_name}"
    if page is not None:
        source += f" · p.{page}"
    sheet_name = _mixed_file_sheet(result, preference)
    if sheet_name:
        source += f" · {sheet_name}"
    answer = scrub_internal_terminology(cleanup_markdown_answer(f"{evidence}\n\n{source}"))
    answer = _restore_file_display_labels(answer, _file_source_items(result))
    return FinalAnswer(
        text=answer,
        charts=[],
        timing={"deterministic_file_render": True, "synthesis_llm_calls": 0},
        ...
        sources=("document",),
        conversation_id=conversation_id,
        file_sources=_preferred_file_source_items(result, preference),
    )
...
def _mixed_file_page(result: dict, context: str, preference: str = "both") -> int | None:
    for item in _preferred_file_source_items(result, preference):
        value = item.get("i_page")
        if isinstance(value, int) and value > 0:
            return value
        if isinstance(value, str) and value.isdigit() and int(value) > 0:
            return int(value)
    if preference == "spreadsheet":
        return None
...
def _mixed_file_name(result: dict, preference: str = "both") -> str:
    for item in _preferred_file_source_items(result, preference):
...
def _mixed_file_sheet(result: dict, preference: str) -> str:
    if preference != "spreadsheet":
        return ""
    for item in _preferred_file_source_items(result, preference):
...
def _file_surface_preference(question: str, result: dict) -> str:
    has_document = any(
        _file_name_has_suffix(item, _DOCUMENT_FILE_SUFFIXES)
        for item in _file_source_items(result)
    )
    has_spreadsheet = any(
        _file_name_has_suffix(item, _SPREADSHEET_FILE_SUFFIXES)
    ...
    if has_document and not has_spreadsheet:
        return "document"
    if has_spreadsheet and not has_document:
        return "spreadsheet"
    document_target = bool(_DOCUMENT_TARGET_RE.search(question))
    spreadsheet_target = bool(_SPREADSHEET_TARGET_RE.search(question))
...
def _preferred_file_source_items(result: dict, preference: str) -> tuple[dict[str, Any], ...]:
    items = _file_source_items(result)
    suffixes = (
        _DOCUMENT_FILE_SUFFIXES
        if preference == "document"
        else _SPREADSHEET_FILE_SUFFIXES
        if preference == "spreadsheet"
        else ()
    )
    preferred = tuple(item for item in items if _file_name_has_suffix(item, suffixes))
    return preferred or items


def _file_name_has_suffix(item: Mapping[str, Any], suffixes: tuple[str, ...]) -> bool:
