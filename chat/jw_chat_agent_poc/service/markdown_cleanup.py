...
_INLINE_LITERAL_RE: Final[re.Pattern[str]] = re.compile(r"`[^`\n]+`")
_VERBATIM_SOURCE_LINE_RE: Final[re.Pattern[str]] = re.compile(
    r"(?m)^[ \t]*(?:[-*][ \t]+)?(?:시트(?:·테이블명)?):[^\n]*$"
)


def _collapse_generated_spacing(text: str) -> str:
    """Collapse prose spacing while preserving verbatim inline source labels."""

    literals: list[str] = []

    def protect(match: re.Match[str]) -> str:
        token = f"\x00INLINE_LITERAL_{len(literals)}\x00"
        literals.append(match.group(0))
        return token

    protected = _VERBATIM_SOURCE_LINE_RE.sub(protect, text)
    protected = _INLINE_LITERAL_RE.sub(protect, protected)
    protected = re.sub(r"(?<=\S)[ \t]{2,}(?=\S)", " ", protected)
    for index, literal in enumerate(literals):
        protected = protected.replace(f"\x00INLINE_LITERAL_{index}\x00", literal)
    return protected
...
def scrub_internal_terminology(text: str) -> str:
    ...
    result = _collapse_generated_spacing(result)
