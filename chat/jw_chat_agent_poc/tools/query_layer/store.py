...
_MART_PERIOD_RE = re.compile(
    r"^(?P<year>\d{4})(?:-(?:(?P<quarter>Q[1-4])|(?P<month>0?[1-9]|1[0-2])))?$",
    re.IGNORECASE,
)


def _mart_period_sort_key(period: str) -> tuple[int, int, int, int, str]:
    value = str(period).strip()
    match = _MART_PERIOD_RE.fullmatch(value)
    if match is None:
    ...
    granularity = 1 if quarter else 0
    return (0, year, sequence, granularity, value.casefold())
...
class MartSnapshot:
    ...
    def periods(self, market_id: str, source: str = "ubist", measure: str = "sales") -> tuple[str, ...]:
        ...
        return tuple(sorted(values, key=_mart_period_sort_key))
        ...
        periods = tuple(
            sorted(
                (
                    ...
                    if self.value_or_none(record, period) is not None
                ),
                key=_mart_period_sort_key,
            )
        )
    ...
    def value_status(self, record: MartRecord, period: str) -> str:
        if len(period) == 4 and period.isdigit():
            statuses = tuple(
                ...
                for key in sorted(record.metric_history, key=_mart_period_sort_key)
