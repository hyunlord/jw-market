...
FailureClass = Literal["timeout", "connect", "5xx", "0_results", "schema", "quota", "none"]
...
def failure_class_from_exception(exc: BaseException) -> FailureClass:
    ...
    if any(token in text for token in ("timeout", "timed out", "deadline exceeded")):
        return "timeout"
    if any(token in text for token in ("connection refused", "failed to establish a new connection")):
        return "connect"
...
def failure_class_from_call(call: _ExternalCallLike) -> FailureClass:
    ...
    if call.status != "error":
        return "none"
    error_type = str(call.render_data.get("error_type") or "")
    if error_type.casefold() == "quota":
        return "quota"
    error = call.render_data.get("error")
    message = call.render_data.get("message")
    return failure_class_from_exception(
        RuntimeError(" ".join(str(value) for value in (error_type, error, message) if value))
    )
