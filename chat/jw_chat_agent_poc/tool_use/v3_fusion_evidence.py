...
_DEFAULT_PROMPT_SEQUENCE_LIMIT = 40
_HIRA_PROMPT_SEQUENCE_LIMIT = 160
...
def fusion_fact_payload(fact: V3EvidenceFact) -> dict[str, object]:
    ...
    raw_result = prompt_value(
        raw_result_value,
        sequence_limit=(
            _HIRA_PROMPT_SEQUENCE_LIMIT
            if fact.tool_name.startswith("hira_")
            else _DEFAULT_PROMPT_SEQUENCE_LIMIT
        ),
    )
...
def prompt_value(
    ...
    *,
    depth: int = 0,
    sequence_limit: int = _DEFAULT_PROMPT_SEQUENCE_LIMIT,
) -> object:
    if depth >= 8:
        return "<nested value omitted>"
    if is_dataclass(value) and not isinstance(value, type):
        return prompt_value(
            asdict(value), depth=depth + 1, sequence_limit=sequence_limit
        )
    if hasattr(value, "model_dump"):
        return prompt_value(
            value.model_dump(), depth=depth + 1, sequence_limit=sequence_limit
        )
    if isinstance(value, Mapping):
        return {
            str(key): prompt_value(
                item, depth=depth + 1, sequence_limit=sequence_limit
            )
            for key, item in list(value.items())[:100]
        }
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        items = [
            prompt_value(item, depth=depth + 1, sequence_limit=sequence_limit)
            for item in list(value)[:sequence_limit]
        ]
