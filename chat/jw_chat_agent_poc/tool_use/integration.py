...
from jw_chat_agent_poc.hira_surface import canonical_hira_disease_query
...
def _disease_query(question: str) -> str | None:
    ...
    canonical = canonical_hira_disease_query(question)
    ...
    for index, token in enumerate(normalized_tokens):
        if len(token) < 2 or not token.endswith(suffixes):
            continue
        if index > 0 and normalized_tokens[index - 1].endswith("성"):
            return f"{normalized_tokens[index - 1]} {token}"
        return token
    patient_subject = re.search(
        r"(?<![가-힣A-Za-z0-9])(?P<subject>[가-힣A-Za-z0-9]{2,40})\s+"
        r"(?:환자\s*수|환자|유병률|진료\s*인원)(?=$|\s|[?.!。？！])",
        question,
        flags=re.IGNORECASE,
    )
    if patient_subject is not None:
        return patient_subject.group("subject")
