...
def _news_analysis(calls: list[Call]) -> Call | None:
    ...
    for call in calls:
        ...
        if not isinstance(items, list):
            continue
        raw_count += sum(isinstance(item, dict) for item in items)
        for item in items:
    ...
    return _analysis(
        ...
        news_raw_count=raw_count,
        news_after_filter_count=len(refs),
        internal_metric_claim_count=1 if metric is not None else 0,
