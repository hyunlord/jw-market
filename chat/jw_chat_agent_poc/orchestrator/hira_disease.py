...
from jw_chat_agent_poc.hira_surface import filter_hira_codes
...
DISEASE_CODE_RE = re.compile(r"(?<![A-Za-z0-9])(?P<code>[A-Za-z]\d{2}(?:[._]?\d)?)(?![A-Za-z0-9])")
...
def hira_direct_disease_calls(question: str, disease_query: str, external: ExternalApiClient) -> list[ExternalCall]:
    ...
    match resolution:
        ...
        case HiraDiseaseCodeAmbiguous(search_call=search_call, candidates=candidates):
            if _is_complete_hira_category_family(candidates):
                return _hira_category_family_calls(
                    question,
                    disease_query,
                    external,
                    search_call,
...
def _is_complete_hira_category_family(
    candidates: tuple[HiraDiseaseCandidate, ...],
) -> bool:
    """Recognize the full visible set of sibling three-character KCD categories."""

    codes = tuple(candidate.sick_cd for candidate in candidates)
    return (
        len(codes) == MAX_HIRA_DISEASE_CANDIDATES
        and all(re.fullmatch(r"[A-Z]\d{2}", code) for code in codes)
        and len({code[:2] for code in codes}) == 1
    )


def _hira_category_family_calls(
    question: str,
    disease_query: str,
    ...
    """Expand a broad KCD family without selecting one candidate as the answer."""

    codes = tuple(candidate.sick_cd for candidate in candidates)
    search = ExternalCall(
        tool=search_call.tool,
        source=search_call.source,
        status=search_call.status,
        summary_text=(
            f"HIRA search_disease_code에서 {disease_query}의 3자리 KCD 질환군 "
            f"{', '.join(codes)}를 확인했습니다."
        ),
        render_data={
            **search_call.render_data,
            "resolved_sickCds": list(codes),
            "resolved_candidate_total": len(candidates),
            "resolution_mode": "complete_three_character_category_family",
        },
    ...
    with ThreadPoolExecutor(
        ...
        thread_name_prefix="hira-category",
    ) as executor:
        futures = [
            executor.submit(
                _hira_stat_external_calls,
                question,
                external,
                candidate.sick_cd,
            )
    ...
    calls: list[ExternalCall] = [search]
    total = len(candidates)
    for index, (candidate, future) in enumerate(
        zip(candidates, futures, strict=True),
        start=1,
    ):
        mapping = _hira_mapping(
            candidate.sick_cd,
            candidate.disease_name,
            f"HIRA search_disease_code 3자리 질환군 전개({disease_query})",
        )
        calls.extend(
            _with_hira_mapping_context(call, disease_query, mapping, index, total)
...
def resolve_hira_disease_code(
    ...
    *,
    ...
    selected_codes = frozenset(
        filter_hira_codes(
            disease_query,
            tuple(candidate.sick_cd for candidate in candidates),
        )
    )
    candidates = tuple(
        candidate for candidate in candidates if candidate.sick_cd in selected_codes
    )
...
def _hira_stat_call(
    ...
    lane_call = call.render_data.get("lane_call")
    if isinstance(lane_call, dict):
        attempts = lane_call.get("attempts", 1)
...
def normalize_hira_disease_code(code: str) -> str:
    compact = code.upper().replace(".", "").replace("_", "")
    if len(compact) == 4:
...
def _hira_code_ambiguous_call(
    ...
    visible_codes = ", ".join(candidate.sick_cd for candidate in visible_candidates)
    return ExternalCall(
        tool="hira_disease_code_ambiguous",
        source="hira_disease",
        status="ambiguous",
        summary_text=(
            f"HIRA search_disease_code에서 {query} 후보({visible_codes})가 여러 건이라 "
            "통계를 조회하지 않았습니다."
        ),
        render_data={
            "query": query,
