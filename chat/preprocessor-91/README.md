...
`preprocessor-91` currently runs the shared image `mnc/doc-parser-preprocessor:1.3.8.1-gpu-libpath-memfix-fv` and mounts `src/preprocessor.py` from the `preprocessor-91` ConfigMap. The candidate Dockerfile layers the same source over that verified base image for immutable build verification; deployment remains a separate PL-controlled operation.
...
- `OCR_TESSERACT_PSM` is the runtime source of truth and defaults to `4` in both code and the candidate image.
- Values outside Tesseract's supported `0..13` range fail during process startup. There is no silent fallback to PSM 3.
- OCR keeps Tesseract OEM 3, `eng+kor`, and the existing PyMuPDF4LLM DPI. Only the page segmentation mode changes.
- Text-layer pages do not install or invoke the OCR adapter, preserving the native fast path.
