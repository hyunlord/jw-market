...
TOOL_DESCRIPTION_CATALOG: tuple[ToolDescriptionRecord, ...] = (
    ...
    ToolDescriptionRecord("openfda_label_search", True, "FDA 라벨과 이상반응 조회. when to use: 부작용/adverse/side effect/FDA 라벨. when NOT to use: 성분/허가/특허."),
    ToolDescriptionRecord("web_search", True, "최신 동향/가이드라인/학회/KOL 웹 검색. when to use: DB에 없는 시의성 정보. when NOT to use: 시장 수치/매출/점유율."),
    ...
    ToolDescriptionRecord("mfds_clinical_trial_kr", True, "국내 임상시험 조회. when to use: 국내 임상/시험계획 질의. when NOT to use: 글로벌 임상만 묻는 질의."),
    ToolDescriptionRecord("clinicaltrials_v2_search", True, "ClinicalTrials.gov 글로벌 임상 조회. when to use: 글로벌 임상/NCT 질의. when NOT to use: 국내 식약처 임상만 묻는 질의."),
