...
INTERNAL_TOOL_DESCRIPTION_CATALOG: tuple[ToolDescriptionRecord, ...] = (
    _record(
        ...
        "내부 mart의 브랜드 단일 지표 조회. when to use: 브랜드 매출·점유율·순위의 특정 기간 값. when NOT to use: 시장 전체 규모나 규제·급여 정보.",
        not_for=("시장 전체 규모", "규제·급여 정보"),
        constraints=("brand·metric·period와 호환 가능한 market·source가 필요함",),
    ...
    _record(
        ...
        "내부 mart의 시장 전체 규모 조회. when to use: 브랜드가 속한 시장의 전체 매출 규모. when NOT to use: 개별 브랜드 매출이나 비교.",
        not_for=("개별 브랜드 매출", "다중 브랜드 비교"),
        constraints=("anchor brand 또는 market으로 시장 범위를 해소해야 함",),
    ...
    _record(
        ...
        "내부 mart의 시장 구성 브랜드 조회. when to use: 시장에 포함된 브랜드 목록과 범위. when NOT to use: 브랜드별 수치나 정의 변경 사유.",
        not_for=("브랜드별 매출 값", "시장 정의 변경 사유"),
        constraints=("anchor brand 또는 market으로 시장을 해소해야 함",),
    ...
    _record(
        ...
        "내부 mart의 브랜드 기간별 추이 조회. when to use: 매출·점유율 등 한 지표의 시계열. when NOT to use: 단일 시점 값이나 미래 예측.",
        not_for=("단일 시점 값", "미래 예측"),
        constraints=("brand·metric·period·source 축이 호환되어야 함",),
    ...
    _record(
        ...
        "내부 mart의 채널별 분해 조회. when to use: 브랜드 매출을 병원·의원 등 채널로 분해. when NOT to use: 진료과 분해나 전체 시장 목록.",
        not_for=("진료과 분해", "시장 구성 브랜드 목록"),
        constraints=("채널 축을 제공하는 source·market 조합이어야 함",),
    ...
    _record(
        "market.get_hhi",
        "내부 mart의 시장 집중도 HHI 조회. when to use: 특정 시장의 경쟁 집중도. when NOT to use: 개별 브랜드 점유율이나 집중도 원인 설명.",
        not_for=("개별 브랜드 점유율", "집중도 원인 추론"),
        constraints=("동일 market·source·period의 브랜드 값으로 계산된 HHI만 사용함",),
    ...
    _record(
        ...
        "내부 mart의 브랜드 성장 기여도 조회. when to use: 시장의 전년 대비 절대 성장에서 브랜드 기여 비중. when NOT to use: 단순 성장률이나 인과 분석.",
        not_for=("단순 성장률", "성장 원인 추론"),
        constraints=("같은 market·source의 비교 가능한 전년 기간 값이 필요함",),
    ...
    _record(
        ...
        "내부 mart의 다중 브랜드 비교 조회. when to use: 같은 시장·소스·지표에서 두 브랜드 이상 비교. when NOT to use: 서로 다른 소스 수치의 직접 비교.",
        not_for=("서로 다른 source의 직접 비교", "시장 정의가 다른 브랜드 비교"),
        constraints=("비교 브랜드가 같은 market·source·metric 축에서 호환되어야 함",),
