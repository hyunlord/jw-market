...
class MarketDefinitionRegistry:
    ...
    def _landscape_definition(self, market_id: str, brand: str) -> dict[str, Any]:
        ...
        statements = [
            f"{market_id}는 MI Master 카탈로그에 기록된 market_landscape 전략뷰 시장입니다.",
            "이 전략뷰의 ATC4 범위는 " + ", ".join(atc4_codes) + "입니다.",
        ]
        if children:
            statements.append(
                "하위 competitive_dynamics 전략시장은 "
                + ", ".join(_text(child.get("cd_id")) for child in children)
                + "입니다."
            )
        result: dict[str, Any] = {
            "definition_type": "market_definition",
            ...
            "market_name": _text(row.get("name")) or market_id,
    ...
    def _competitive_definition(self, market_id: str, brand: str) -> dict[str, Any]:
        ...
        statements = [
            f"{market_id}는 {parent_id} market_landscape에서 범위를 좁힌 competitive_dynamics 전략뷰 시장입니다.",
            f"좁힘 규칙의 식별자는 {rule_id}입니다. 조건 본문은 현재 런타임 카탈로그에서 조회되지 않습니다.",
        ]
        result: dict[str, Any] = {
            "definition_type": "market_definition",
            "view_category": "전략뷰",
            ...
            "parent_market_name": _text((parent or {}).get("name")) or parent_id,
            "data_source": _source_label(row.get("data_source")),
            "analysis_dimensions": _analysis_dimensions(row),
            "narrowing_rule": {
                "available": False,
    ...
    def _competitive_children_definition(self, parent_market_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "parent_market_name": _text(parent.get("name")) or parent_market_id,
            "competitive_market_identifiers": child_ids,
            "definition_statements": [
                f"{parent_market_id} market_landscape에서 갈라지는 competitive_dynamics 전략뷰 시장은 "
                + ", ".join(child_ids)
                + "입니다."
            ],
            "selection_rationale": _unavailable_rationale(),
