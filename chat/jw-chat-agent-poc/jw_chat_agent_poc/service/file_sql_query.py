...
def query_uploaded_sql(
    ...
    try:
        ...
        context = _render_result(source, result)
        return SqlQueryOutcome(
...
def _render_result(source: SqlFileSource, result: Mapping[str, Any]) -> str:
    columns = result.get("columns")
    rows = result.get("rows")
    safe_columns = [str(value) for value in columns] if isinstance(columns, list) else []
