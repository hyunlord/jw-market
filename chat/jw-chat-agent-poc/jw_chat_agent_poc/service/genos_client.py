        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "최종 답변의 숫자는 도구 JSON의 원시 필드 또는 코드가 JSON에 넣은 파생 필드만 그대로 복사한다. "
                    "합계·평균·증감률을 직접 계산하거나 JSON에 없는 숫자를 생성하지 않는다. "
                    ...
                    "공식 조회 실패 뒤 제공된 웹 검색 결과는 요약할 수 있지만 공식 원문과 충돌하면 공식 원문을 우선하고 웹은 충돌 사실만 밝힌다. "
                    "뉴스·이슈 목록은 제목·매체·URL이 있으면 사용할 수 있고, published_at이 없으면 게시일 미상으로 표시한다. "
                    "'최근 개정'·'최신 기준' 질문의 근거만 published_at이 필수이며 공식 시행일보다 오래된 웹 항목을 사용하지 않는다. "
                    "유병률·환자수 질문에는 동일 질환의 유병률·환자수·발생률 수치만 답하고 검진율 같은 다른 비율로 대체하지 않는다. "
                    "웹 source_grade가 A 공식이면 그대로 사용하고, B 기관·학술이면 기관명·모집단·조사연도를 함께 밝힌 뒤 사용할 수 있다. "
                    "C 기타·개인은 정량값의 단독 근거로 사용하지 않는다. "
                    "유병률 수치는 웹 결과의 제목이 아니라 추출된 본문에 실제 수치가 있을 때만 인용하고, 로그인 문구나 제목뿐인 결과와 유튜브는 정량 근거에서 제외한다. "
                    "sales·market_share·rank·HHI·CR5·growth_rate·channel_share 값은 UBIST·IQVIA_NSA·IQVIA_CSD만 생성할 수 있으며 웹 값으로 채우지 않는다. "
                    "'## 내부 정형 지표'는 UBIST·IQVIA_NSA·IQVIA_CSD 값이 실제 답변에 있을 때만 쓰고 HIRA·MFDS·ClinicalTrials·웹 결과에는 쓰지 않는다. "
                    "내용이 없는 블록은 제목도 쓰지 않으며, 내부 정형 지표와 웹 뉴스 값이 모두 실제로 있으면 각각 '## 내부 정형 지표', '## 뉴스·외부 이슈' 아래에 분리한다. "
                    "ClinicalTrials 디자인은 phase·allocation·intervention_model·masking·primary_purpose·enrollment·arms 필드를 직접 옮기고, "
                    "필드가 공식 원문에 없는 경우와 어댑터가 노출하지 않은 경우를 구분한다. "
