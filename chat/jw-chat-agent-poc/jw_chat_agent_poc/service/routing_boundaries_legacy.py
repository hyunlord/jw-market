...
def legacy_app_scope_decision(
    *,
    ...
    file_schema_columns: Iterable[str],
    ...
    schema_columns = tuple(file_schema_columns)
    context_scope = resolve_context_scope_fn(
        ...
        file_schema_columns=schema_columns,
    )
    if needs_brand_clarification or needs_market_clarification:
        context_scope = ContextScope.MARKET
    file_schema_match = matches_file_schema_fn(file_question, schema_columns)
...
def legacy_market_shortcut_decision(
    *,
    question: str,
    has_documents: bool,
