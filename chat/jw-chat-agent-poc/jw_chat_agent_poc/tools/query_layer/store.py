...
def mart_ttl_seconds() -> int:
    ...
    Every caller must agree on this value. The shared store used to be keyed by
    whatever TTL the first caller happened to pass, and the three call sites read
    three different things (``CHAT_QUERY_MART_TTL_SECONDS``,
    ``CHAT_MARKET_SCOPE_TTL_SECONDS``, and a hardcoded default). They currently all
    resolve to 300, so one store exists -- but changing only the mart env would have
    produced a second store, and a second store means a second full snapshot
    resident at once. Reading the TTL in one place makes that impossible.
    """
...
class MariaDbStrategicMartReader:
    ...
    def fingerprint(self) -> str | None:
        ...
        runs. This probe answers "did the rows change at all?" for ~40ms, so the
        expensive rebuild only has to happen when the answer is yes. Returning None
        means "unknown", and the caller must then fall back to a full rebuild.
        """
        import pymysql
...
class TtlStrategicMartStore:
    def __init__(
        ...
        # its __post_init__ rebuilds DerivedSnapshotIndex, so replacing it just to move
        # a timestamp would re-spend the build. Only one place ever read
        # snapshot.loaded_at, and it is the TTL comparison below.
        self._loaded_at = 0.0
    ...
    def _refresh_snapshot(self) -> None:
        ...
        # Rebuilding holds the new snapshot alongside the one still being served, so
        # for the 32-38s the load takes the process carries two full copies. Ask first
        # whether the rows actually changed; when they have not, extending freshness is
        # equivalent to rebuilding and costs no memory. A None probe means "unknown"
        # and falls through to the rebuild, preserving the original behaviour.
        probe = self._probe_fingerprint()
...
def shared_strategic_mart_store() -> TtlStrategicMartStore:
    ...
    of them is not a caching trade-off, it is the difference between running and being
    OOM-killed. The TTL comes from mart_ttl_seconds() so no caller can accidentally
    mint a second store by passing a different number.
    """
