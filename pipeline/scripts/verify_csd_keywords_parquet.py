"""
verify_csd_keywords_parquet.py
==============================
Verify parquet/csd_keywords/*.parquet against IQVIA CSD Keywords raw xlsx.
...
Usage:
    python3 scripts/verify_csd_keywords_parquet.py \\
        --input-dir "data/IQVIA/CSD/Keyword (고객=의사에게 전달한 메세지)" \\
        --parquet-dir parquet/csd_keywords
"""
...
from prototype_04_csd_keywords_to_parquet import (  # noqa: E402
    OUTPUT_COLS,
    iter_csd_keyword_rows,
    resolve_input_files,
)
...
def count_raw_rows(path: Path) -> int:
    return sum(1 for _ in iter_csd_keyword_rows(path))
...
def build_expected_records(path: Path, targets: set[tuple[str, int]]) -> dict[tuple[str, int], dict[str, Any]]:
    ...
    for record in iter_csd_keyword_rows(path, ingested_at="__VERIFY__"):
...
def phase3_raw_json_validity(parquet_dir: Path) -> tuple[int, int, dict[str, int]]:
    ...
    print(f"  Keywords rows:         {keywords_rows:,}")
...
def main() -> None:
    ...
    parser.add_argument("--parquet-dir", default="parquet/csd_keywords")
    ...
    print("=" * 72)
    print("Verify IQVIA CSD Keywords Parquet")
    print("=" * 72)
