...
def text_sha256(text: str) -> str:
    """Hash normalized source text without persisting the text itself."""
    return hashlib.sha256(" ".join(text.split()).encode("utf-8")).hexdigest()


def estimate_tokens(text: str) -> int:
    """Estimate Korean-heavy prompt tokens from character count."""
    return max(1, round(len(text) / 2.2))


def redacted_rows_for_audit(rows: list[KeywordRow]) -> list[dict[str, JsonValue]]:
    """Render source rows for audit without raw keyword text."""
    return [
        {
            "row_id": row.row_id,
