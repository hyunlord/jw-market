#!/usr/bin/env python3
"""Rebuild strategic brand catalogs with explicit canonical JW brand rows.

The source market catalogs are product/ingredient oriented in several places.
Layer 3 marts, however, need a stable Korean brand grain aligned to the public
API specification.  This script keeps the existing market-member rows and adds
one explicit ``is_jw=1`` row for each of the 25 canonical JW brands.
"""
    ...
    *,
    ...
    canonical_rows = [
        _canonical_row(
            ...
            id_col=id_col,
            row_index=index,
            brand_id_prefix=brand_id_prefix,
        )
        for index, spec in enumerate(CANONICAL_BRANDS, start=1)
    ]
    canonical_df = pd.DataFrame(canonical_rows, columns=result.columns)
    result = pd.concat([canonical_df, result], ignore_index=True)
...
def verify_canonical(df: pd.DataFrame) -> None:
    ...
    actual = set(df.loc[df["is_jw"] == True, "name"].astype(str))  # noqa: E712 - pandas mask
    missing = expected - actual
    extra = actual - expected
    if missing or extra:
        raise RuntimeError(f"Canonical mismatch. missing={sorted(missing)} extra={sorted(extra)}")
    if int((df["is_jw"] == True).sum()) != 25:  # noqa: E712 - pandas mask
...
def main() -> int:
    ...
    print("strategic_brand rows:", len(strategic_brand))
    print("strategic_brand canonical rows:", int((strategic_brand["is_jw"] == True).sum()))  # noqa: E712
    print("cd_brand rows:", len(cd_brand))
    print("cd_brand canonical rows:", int((cd_brand["is_jw"] == True).sum()))  # noqa: E712
    if args.verbose:
        print(strategic_brand.loc[strategic_brand["is_jw"] == True, ["ml_id", "cd_id", "name", "general_brand_key", "is_target"]].to_string(index=False))  # noqa: E712
    return 0
