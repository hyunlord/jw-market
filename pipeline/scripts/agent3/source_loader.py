...
@dataclass(frozen=True, slots=True)
class HashMigrationSourceState:
    brand_key: str
    source: Agent3Source
    brand_name: str
    serving_brand_name: str | None
...
def make_source_record(
    *,
    ...
    return Agent3SourceRecord(
        ...
        input_hash=compute_source_input_hash(profile, input_candidates, workflow_rev, source),
...
class Agent3SourceLoader:
    ...
    def load_coverage(self) -> tuple[int, int, int]:
        with connect(self.config) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT COUNT(*) AS source_units,
                           COUNT(DISTINCT brand_key) AS brands,
                           SUM(CASE
                                 WHEN JSON_LENGTH(JSON_EXTRACT(strength_summary_json, '$.strength_items')) = 0
                                 THEN 1 ELSE 0
                               END) AS profile_only
                ...
                row = cursor.fetchone()
        return int(row["source_units"]), int(row["brands"]), int(row["profile_only"] or 0)
    ...
    def load_existing_hashes(self, brand_keys: list[str]) -> dict[tuple[str, Agent3Source], ExistingAgent3SourceState]:
        ...
        with connect(self.config) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    f"""
                    SELECT brand_key, source, input_hash, workflow_rev, profile_json,
                           strength_candidates_json, strength_summary_json
                    FROM agent3_brand_strength_source
    ...
    def iter_market_position_states(self, *, batch_size: int = 1_000):
        offset = 0
        while True:
            with connect(self.config) as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        """
                        SELECT brand_key, source, brand_name, serving_brand_name, profile_json,
                               strength_candidates_json, strength_summary_json, workflow_id,
                               workflow_rev, input_hash
                        FROM agent3_brand_strength_source
                        WHERE JSON_UNQUOTE(JSON_EXTRACT(strength_candidates_json, '$[0].metric')) = 'market_position'
                        ORDER BY brand_key, source
                        LIMIT %s OFFSET %s
                        """,
                        (batch_size, offset),
                    )
            ...
            if not rows:
                return
            for row in rows:
                yield HashMigrationSourceState(
                    brand_key=str(row["brand_key"]),
                    source=_parse_source(str(row["source"])),
                    brand_name=str(row["brand_name"]),
                    serving_brand_name=(
                        str(row["serving_brand_name"]) if row.get("serving_brand_name") is not None else None
                    ),
                    profile_json=_json_object(row["profile_json"]),
                    strength_candidates_json=_json_list(row["strength_candidates_json"]),
                    strength_summary_json=_json_object(row["strength_summary_json"]),
...
def canonical_content_matches(old: ExistingAgent3SourceState, new: Agent3SourceRecord) -> bool:
