"""
prototype_14_dim_market_landscape_to_parquet.py
===============================================
Phase 12 Round 4 dim_market_landscape -> Parquet.
...
Policy:
- This table is newly defined by Phase 11 Step C-2.
- The grain is one row per strategic market sheet (16 rows).
- market_landscape_id is deterministic: ml_001 through ml_016 by strategy id.
- ml_brand_count and ml_brand_list_json are precomputed from corrected
  master_drug rows.
...
def metric_json_from_analysis_levels(analysis_levels_json: Any) -> str:
    ...
    return dumps_json(analysis_levels.get("Metrics", []))
...
def write_parquet(records: list[dict[str, Any]], output_file: Path) -> None:
    ...
    schema = pa.schema([pa.field(column, pa.string()) for column in DIM_MARKET_LANDSCAPE_COLUMNS])
    rows = [
        {column: None if record.get(column) is None else str(record.get(column)) for column in DIM_MARKET_LANDSCAPE_COLUMNS}
        for record in records
    ]
...
def main() -> None:
    args = parse_args()
    records = load_dim_market_landscape_records(args.market_definition, args.master_drug)
    ...
    for definition_type, count in sorted(_count_by(records, "ml_definition_type").items()):
