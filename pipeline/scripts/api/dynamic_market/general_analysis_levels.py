...
def build_general_analysis_level_sections(
    *,
    ...
    if source_api == "UBIST":
        ubist_channel_context = _aggregated_ubist_channel_context(metrics)
        if ubist_channel_context is None:
    ...
    analysis_levels = project_analysis_level_channels(all_channel_levels, channels)
    ...
    market_status_levels = project_analysis_level_channels(all_channel_levels, status_channels)
...
def _aggregated_ubist_channel_context(metrics: AggregatedMetrics) -> dict[str, Any] | None:
    if not metrics.ubist_specialty_channels:
        return None
    return {
...
def project_analysis_level_channels(payload: dict[str, Any], channels: list[str]) -> dict[str, Any]:
    """Project a once-built analysis payload onto a channel subset without recomputation."""
