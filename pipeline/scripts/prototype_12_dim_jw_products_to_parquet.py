"""
prototype_12_dim_jw_products_to_parquet.py
==========================================
...
- Output schema has no prototype helper columns such as source_files or period.

Usage, in Step D after user review:
    python3 scripts/prototype_12_dim_jw_products_to_parquet.py
"""
...
def _source_file_version_from_market_definition(
    market_definition_records: list[dict[str, Any]],
) -> str:
    versions = {
...
def write_parquet(records: list[dict[str, Any]], output_file: Path) -> None:
    ...
    schema = pa.schema([pa.field(column, pa.string()) for column in DIM_JW_PRODUCTS_COLUMNS])
    rows = [
        {column: None if record.get(column) is None else str(record.get(column)) for column in DIM_JW_PRODUCTS_COLUMNS}
        for record in records
    ]
...
def main() -> None:
    ...
    for market_id, count in sorted(_count_by(records, "strategic_market_id").items()):
