"""Provision isolated schemas for an R-1 full-pipeline rehearsal."""
...
class FullRehearsalProvisionConfig:
    host: str
    port: int
    root_password: str
    writer_user: str
...
def _validate(config: FullRehearsalProvisionConfig) -> None:
    if not config.host:
...
def _statements(config: FullRehearsalProvisionConfig) -> tuple[str, ...]:
    statements: list[str] = []
...
def provision_full_rehearsal_databases(
    config: FullRehearsalProvisionConfig,
    *,
