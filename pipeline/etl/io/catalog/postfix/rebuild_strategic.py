...
def strategic_brand_contract(
    registry: MiMasterRegistry | None = None,
) -> tuple[int, int]:
    active_registry = registry or default_mi_master_registry()
    return len(active_registry.market_sheets), len(active_registry.target_brands)


EXPECTED_ML_MARKET_COUNT, EXPECTED_JW_CANONICAL_ROWS = strategic_brand_contract()
...
def validate_strategic_brand(
    ...
    expected_market_count, expected_jw_count = strategic_brand_contract(registry)
    if len(counts) != expected_market_count:
        raise ValueError(f"expected {expected_market_count} ml markets, found {len(counts)}")
    jw_count = int(catalog["is_jw"].astype(bool).sum()) if "is_jw" in catalog.columns else 0
    if jw_count != expected_jw_count:
        raise ValueError(f"expected {expected_jw_count} JW canonical rows, found {jw_count}")
