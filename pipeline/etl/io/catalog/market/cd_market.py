...
from pipeline.etl.io.catalog.market.cd_market_records import load_cd_market_records
from pipeline.etl.io.catalog.market.cd_market_schema import (
    CD_MARKET_COLUMNS,
    CD_MARKET_SCHEMA,
    DEFAULT_CD_FILTER_FILE,
    DEFAULT_MARKET_DEFINITION_FILE,
    DEFAULT_ML_MARKET_FILE,
    DEFAULT_OUTPUT_FILE,
)
from pipeline.etl.io.catalog.market.cd_market_schema import count_true, nonnull_count, validate_records
from pipeline.etl.io.catalog._lib.catalog_parquet import validate_written_schema, write_typed_parquet
from pipeline.etl.io.catalog._lib.catalog_text import read_parquet_rows


def write_parquet(records: list[dict[str, Any]], output_file: Path) -> None:
    write_typed_parquet(records, output_file, CD_MARKET_SCHEMA)


def validate_written_parquet(output_file: Path) -> None:
    records = validate_written_schema(output_file, CD_MARKET_SCHEMA)
