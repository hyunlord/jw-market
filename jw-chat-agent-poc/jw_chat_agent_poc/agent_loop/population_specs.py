...
def strict_query_plan(question: str, brand: str) -> StrictQueryPlan | None:
    ...
    if _asks_channel_distribution(question):
...
def _asks_channel_distribution(question: str) -> bool:
    if "채널" not in question:
        return False
    return any(token in question for token in ("어느", "어디", "잘 팔", "많이", "매출", "판매"))
