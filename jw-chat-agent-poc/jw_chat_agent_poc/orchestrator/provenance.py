...
def _metric_values(data: dict[str, Any]) -> list[tuple[str, str, str]]:
    ...
    rows.extend(
        (
            ...
            ("브랜드 CAGR", pct_value(data.get("brand_cagr_5y_pct")), "render_data.brand_cagr_5y_pct"),
            ("시장 CAGR", pct_value(data.get("market_cagr_5y_pct")), "render_data.market_cagr_5y_pct"),
            ("Excess growth", pct_value(data.get("excess_growth_pct")), "render_data.excess_growth_pct"),
    ...
    for call_index, call in enumerate(calls):
        ...
        for item_index, item in enumerate(items(render_data)):
            ...
            if count is not None:
                rows.append(("환자수", str(count), f"render_data.calls[{call_index}].render_data.items[{item_index}].ptntCnt"))
