...
from jw_chat_agent_poc.service.answer_safety import chunk_text, cleanup_markdown_answer, finalized_fallback_fact_answer
...
def create_app(
    *,
    ...
    def chat_stream(
        ...
        item = _resolve_session(store, resolver, make_agent, session_id, question, external_mode, conversation_id)
...
def _resolve_session(
    ...
    if session_id:
        ...
        return item
    if question:
        return _answer_question(store, market_scope_resolver, agent_factory, question, external_mode, conversation_id)
...
def _answer_question(
    ...
    result = _answer_with_conversation(store, market_scope_resolver, agent_factory, state.conversation_id, question, external_mode, documents)
...
def _answer_with_conversation(
    ...
    if pending is not None and pending.kind == "market_view":
        ...
        result = _answer_without_pending(market_scope_resolver, agent_factory, conversation_id, question, external_mode, documents, store)
        return _prepend_pending_notice(result)
    return _answer_without_pending(market_scope_resolver, agent_factory, conversation_id, question, external_mode, documents, store)
