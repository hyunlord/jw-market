...
def _parse_top_trend_line(line: str) -> dict[str, str]:
    match = re.search(
        r"상위 브랜드 추이:\s*(?P<rank>\d+)위\s+(?P<brand>.+?)\s+최신 시장점유율\s+"
...
def _movement_phrase(line: str) -> str:
    ...
    faller = _regex_value(line, r"([가-힣A-Za-z0-9+._/-]+)\s+하락폭")
...
def _presentable_hira_patient_line(line: str) -> str:
    ...
    if not match:
        return ""
    data = match.groupdict()
    return f"HIRA 기준 {data['disease']}({data['code']}) {data['segment']} 환자수는 {data['count']}명입니다."
...
def _hira_patient_summary_lines(fact_md: str) -> tuple[str, ...]:
    ...
    for raw in fact_md.splitlines():
        ...
        if not in_table or not stripped.startswith("|") or "---" in stripped or "구분" in stripped:
            continue
        cells = [cell.strip() for cell in stripped.strip("|").split("|")]
        if len(cells) >= 4 and cells[0] and cells[1] and cells[2] and cells[3] not in {"", "-"}:
...
def _hira_patient_fact_summary(fact_md: str) -> dict[str, str]:
    ...
    segments = ", ".join(
        f"{row['disease']}({row['code']}) {row['segment']} {row['count']}명" for row in selected[:2]
    )
