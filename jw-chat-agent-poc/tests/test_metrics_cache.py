...
def test_chat_agent_reports_unsupported_hospital_level_sales_filter() -> None:
    reader = StaticMetricsCacheReader(cache_brands=CACHE_BRANDS, market_status=BRAND_CARDS)
    tool = MetricsTool(mode="cache", cache_reader=reader, cause_reader=CAUSE_READER)
    ...
    assert data["unsupported_filters"][0]["field"] == "granularity"
    assert "지원" in result["answer"]
    assert "세부 구분" in result["answer"]
