...
def test_generic_channel_question_maps_to_channel_distribution_query() -> None:
    """Given a generic channel question, strict query returns brand channel distribution."""

    planner = ScriptedPlanner((AgentDecision(final_answer="done"),))
    agent = ToolUseAgent(metrics=_metrics_tool(), resolver=BrandResolver(), planner=planner, query_layer=_query_layer())

    result = agent.answer("리바로 어느 채널에서 잘 팔려?")
