...
def test_metric_data_dedupes_market_context_across_metric_tables() -> None:
    ...
    assert response.markdown.count("| 시장 CAGR | 10.73% |") == 1
    assert "| 매출 | 84.93억원 |" in response.markdown
    assert "| 브랜드 CAGR | 4.98% |" in response.markdown
...
def test_get_disease_stats_call_renders_hira_patient_facts_in_body() -> None:
    ...
    assert "본태성 고혈압(I10) 외래: 3769201명" in fact_md
    assert "본태성 고혈압(I10) 입원: 18136명" in fact_md
    assert "### HIRA 질병통계 fact" in fact_md
    assert "| 외래 | I10 | 본태성 고혈압 | 3769201 |" in fact_md
    assert "| 입원 | I10 | 본태성 고혈압 | 18136 |" in fact_md
...
def test_hira_required_rows_prefer_high_level_segments_over_age_tail() -> None:
    ...
    assert "- HIRA 환자수: 본태성(원발성) 고혈압(I10) 입원: 16171명" in lines
    assert "- HIRA 환자수: 본태성(원발성) 고혈압(I10) 외래: 3769201명" in lines
...
def test_answer_facts_prefer_top_trend_when_snapshot_segments_are_zero() -> None:
    ...
    assert "| 상위 브랜드 추이 | 1위 훼로바유 최신 시장점유율 25.85%" in fact_md
    assert "| 상위 브랜드 추이 | 2위 페린젝트 최신 시장점유율 25.36%" in fact_md
...
def test_hira_required_patient_facts_dedupe_same_label_and_disease() -> None:
    ...
    assert mandatory.count("본태성(원발성) 고혈압(I10) 입원") == 1
