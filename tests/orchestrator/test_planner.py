...
def test_refresh_profiles_exclude_the_separate_general_cache_track(tmp_path):
    numeric = build_plan(
        mode="incremental",
        run_id="numeric",
        probe=FakeProbe(),
    ...
    assert agent_keys == {"forecast", "strength", "shortlong", "events", "elements"}
    assert not numeric_keys & agent_keys
    assert numeric_keys | agent_keys == set(STAGE_ORDER) - {"cache"}
